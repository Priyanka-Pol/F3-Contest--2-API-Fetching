...
function fetchingDataFromApi1(apiUrl, delay) {
...
function fetchingDataFromApi2(apiUrl, delay) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            fetch(apiUrl)
                .then(response => response.json())
...
function fetchingDataFromApi3(apiUrl, delay) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            fetch(apiUrl)
                .then(response => response.json())
...
function fetchingApiData() {
    ...
    const api1Promise = fetchingDataFromApi1(api1Url, 1000);
    const api2Promise = api1Promise.then(() => fetchingDataFromApi2(api2Url, 2000));
    const api3Promise = api2Promise.then(() => fetchingDataFromApi3(api3Url, 3000));
...
fetchDataButton.addEventListener("click", fetchingApiData);